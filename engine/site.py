...
nav a{padding:.42rem .66rem;border-radius:999px;white-space:nowrap;
color:var(--mut);transition:.2s}
nav a:hover,nav a[aria-current="page"]{color:#fff;background:rgba(124,77,255,.2)}
.nav-menu{position:relative}
.nav-menu>summary{list-style:none;cursor:pointer;padding:.42rem .66rem;
...
backdrop-filter:blur(14px);box-shadow:0 18px 40px rgba(0,0,0,.5)}
.nav-drop a{padding:.5rem .7rem}
@media(max-width:760px){.nav-drop{position:static;box-shadow:none;
background:rgba(255,255,255,.03);margin:.2rem 0}}
...
background:#0a1420;color:#eae6ff;font-size:.9rem;font-family:system-ui,sans-serif;
text-transform:none;letter-spacing:0;color-scheme:dark}
.reco-row option{background:#0a1420;color:#eae6ff}
...
/* ---- readable docs ---- */
.doc{max-width:54rem;font-size:1.02rem}
.doc h2{font-size:clamp(1.5rem,3vw,2rem)}
.doc h3{font-size:1.2rem}
.doc p{margin:.9rem 0}.doc ul{margin:.9rem 0}
.doc>p:first-of-type,.doc .doc-lead{font-size:1.15rem;color:#e7e2f6;line-height:1.55}
.doc table{font-size:.92rem}
.doc th{background:rgba(124,77,255,.1)}
.doc tbody tr:hover{background:rgba(124,77,255,.06)}
/* ---- flow diagram (native, themeable) ---- */
.flow{display:grid;gap:0;margin:1.6rem 0 2.6rem;max-width:44rem}
.flow-node{display:grid;grid-template-columns:2.6rem 1fr auto;gap:.9rem;
align-items:center;padding:.9rem 1.1rem;border:1px solid var(--line);
border-radius:14px;background:linear-gradient(150deg,rgba(124,77,255,.16),rgba(10,10,22,.7))}
.flow-node .fn{display:grid;place-items:center;width:2.6rem;height:2.6rem;
border-radius:50%;font-family:var(--fm);font-weight:800;color:#08131f;
background:radial-gradient(circle at 38% 30%,var(--gold-soft),var(--violet) 70%)}
.flow-node b{display:block;color:#fff;font-family:var(--fd);font-size:1.05rem}
.flow-node span{color:var(--mut);font-size:.9rem}
.flow-node .fs{font-family:var(--fm);font-size:.62rem;text-transform:uppercase;
letter-spacing:.06em;padding:.15rem .5rem;border-radius:999px;white-space:nowrap}
.flow-node .fs.built{background:rgba(43,227,194,.16);color:var(--teal);border:1px solid rgba(43,227,194,.4)}
.flow-node .fs.designed{background:rgba(201,166,255,.16);color:var(--amethyst);border:1px solid rgba(201,166,255,.4)}
.flow-node .fs.gate{background:rgba(255,207,110,.18);color:var(--gold-soft);border:1px solid rgba(255,207,110,.5)}
.flow-node.gate{border-color:rgba(255,207,110,.5)}
.flow-arrow{justify-self:center;color:var(--gold);font-size:1.1rem;line-height:1;
padding:.28rem 0}
.flow-hero{width:100%;border-radius:16px;border:1px solid var(--line);
margin:1.4rem 0 2rem;display:block}
/* ---- footer sitemap ---- */
.foot-nav{display:grid;grid-template-columns:repeat(auto-fit,minmax(9.5rem,1fr));
gap:1.2rem 1.4rem;width:100%;padding-bottom:1.4rem;margin-bottom:1.3rem;
border-bottom:1px solid var(--line)}
.foot-nav h4{margin:0 0 .5rem;font-family:var(--fm);font-size:.7rem;
text-transform:uppercase;letter-spacing:.1em;color:var(--gold)}
.foot-nav ul{list-style:none;margin:0;padding:0;display:grid;gap:.28rem}
.foot-nav a{color:var(--mut);font-size:.88rem;text-decoration:none}
.foot-nav a:hover{color:#fff}
/* ---- floating to-top ---- */
.to-top{position:fixed;right:1.1rem;bottom:1.1rem;z-index:70;width:2.8rem;
height:2.8rem;border-radius:50%;border:1px solid var(--line);cursor:pointer;
background:rgba(12,12,26,.9);backdrop-filter:blur(12px);color:var(--gold);
font-size:1.1rem;display:grid;place-items:center;opacity:0;transform:translateY(12px);
...
<a href="{up}recon.html">Recon</a></div></details>
...
<button class="to-top" aria-label="Back to top" title="Back to top">&uarr;</button>
<footer class="site-footer"><div class="wrap">
<div class="foot-nav">
<div><h4>Pipeline</h4><ul>
<li><a href="{up}index.html">Progress</a></li>
...
<li><a href="{up}frontier.html">Frontier</a></li>
<li><a href="{up}loras.html">LoRAs</a></li>
<li><a href="{up}compute.html">Compute</a></li>
...
the song vault as pieces complete. Lyrics live with the music, not here.
Luke &times; Claude.</p>
</div></footer>
<script>
...
if(tt)tt.classList.toggle('show',y>500)}}
addEventListener('scroll',s,{{passive:true}});s();
if(tt)tt.addEventListener('click',function(){{scrollTo({{top:0,behavior:'smooth'}})}});
// close an open nav menu when clicking elsewhere
document.addEventListener('click',function(e){{
document.querySelectorAll('.nav-menu[open]').forEach(function(m){{
if(!m.contains(e.target))m.removeAttribute('open')}})}});
// reveal on scroll, and kintsugi seams drawing themselves.
// Deliberately NOT IntersectionObserver: content visibility must never
// depend on an observer callback arriving, because browsers throttle or
// delay IO in background tabs, prerendered pages and low-power modes,
// which can leave a whole page at opacity 0 for the visit. A plain
// geometry sweep on scroll and resize cannot strand content.
var rp=[].slice.call(document.querySelectorAll('.pc-card,.card,.reveal,.seam'));
function rshow(el){{el.classList.add(
el.classList.contains('seam')?'is-drawn':'is-visible')}}
...
function rsweep(){{if(!rp.length)return;var line=innerHeight*0.92;
rp=rp.filter(function(el){{var r=el.getBoundingClientRect();
if(r.top<line){{rshow(el);return false}}
return true}})}}
rsweep();
addEventListener('scroll',rsweep,{{passive:true}});
addEventListener('resize',rsweep,{{passive:true}});
addEventListener('load',rsweep);
// late layout: fonts and images shift geometry after first paint
setTimeout(rsweep,400);setTimeout(rsweep,1500);
...
def _pipeline_flow() -> str:
    ...
    stages = [
        ("Sources", "Lyrics, Suno stems, human cockpit, all stay on trusted machines", "built"),
        ("Ingestion", "Lyric structure plus audio timing and energy into one feature block", "designed"),
        ("The note", "vault/&lt;slug&gt;.md, the song's single source of truth", "built"),
        ("Analyse", "LLM brief: per-section set, scene, energy, transition", "built"),
        ("Gate", "Luke approves the direction and picks treatments", "gate"),
        ("Treatments", "Karaoke video, artistic film, complex film, each its own status", "designed"),
        ("Format renders", "One master to every platform size from one source", "designed"),
        ("Release pack", "Agent-ready folder per song, asset-guide families", "designed"),
        ("Platforms", "YouTube, Shorts, IG, TikTok, Spotify Canvas, Bandcamp, site", "designed"),
    ]
    label = {"built": "built", "designed": "designed", "gate": "your gate"}
    parts = []
    for i, (title, sub, st) in enumerate(stages, 1):
        if i > 1:
            parts.append('<div class="flow-arrow">&darr;</div>')
        cls = " gate" if st == "gate" else ""
        parts.append(
            f'<div class="flow-node{cls}"><span class="fn">{i}</span>'
            f'<div><b>{title}</b><span>{sub}</span></div>'
            f'<span class="fs {st}">{label[st]}</span></div>')
...
def render_site(notes: list, catalogue: dict, out_dir: Path) -> list[Path]:
    ...
    pipeline_body = (
        ...
        "<p>The vault, the ideation loop, this monitor, and the whole job "
        "loop are live. <code>engine make</code> assembles a real job folder "
        "(data, model, compute, direction) for any analysed song; "
        "<code>engine work</code> is the render worker &mdash; it turns each "
        "shot into a ComfyUI graph from a swappable recipe and submits it to "
        "a ComfyUI server on the GPU box, landing frames in the job's "
        "results with a manifest. An offline mode emits those exact graphs "
        "without a GPU, so the loop is inspectable end to end here before any "
        "metal is rented. <code>engine advance</code> moves a song through "
        "the stages as pieces land.</p>"
        "<p class='muted'>Still to plug in behind the same job folder: video "
        "generation, voice and lip-sync, and the beat-synced assembly &mdash; "
        "each a new render recipe and, where needed, a new graph builder, "
        "never a change to the loop above.</p>"
    ...
    for md_name, (html_name, title) in rendered_docs.items():
        ...
        if md_name == "PLAN.md":
            # Luke's diagram image is the hero when present; the native
            # vector flow is the fallback so the page is never bare.
            if (out_dir / "assets" / "pipeline.webp").exists():
                hero = ('<img class="flow-hero" src="assets/pipeline.webp" '
